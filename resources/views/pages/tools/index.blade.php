...
<header>
    <div class="mx-auto max-w-9xl pt-2 pb-6 px-4">
        <h1 class="text-3xl font-bold tracking-tight">Daftar Alat</h1>
    </div>
</header>
<main>
    <div class="mx-auto max-w-9xl pb-6 px-4">
        <div class="space-y-3">
            <div class="sm:flex-none space-x-1">
                <a href="{{ route('tool.create') }}" class="m-0 inline-flex items-center justify-center border hover:text-rose-600 hover:bg-rose-50 px-3 py-2 rounded-lg text-xs font-semibold">Tambah Data</a>
            </div>
                                        ...
                                        @isset($data)
                                            <tr>
                                                ...
                                                <td class="whitespace-nowrap px-4 py-2 text-xs">
                                                    <form class="m-0" action="{{ route('tool.destroy', $data->id) }}" method="POST">
                                                        <a href="{{ route('tool.show', $data->id) }}" class="hover:text-rose-600 font-semibold">Info</a>
                                                        <a href="{{ route('tool.edit', $data->id) }}" class="ml-1 hover:text-rose-600 font-semibold">Ubah</a>
                                                        @csrf
                                                        @method('DELETE')
